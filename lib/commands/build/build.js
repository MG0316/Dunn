...
const pathSrc = path.join(cwd, '/src/'); // Common path based on file location:
...
function loadStyles() {
  function applyPlugins(file, fname) {
    // const { callback } = require(`${pathSrc}plugins/${config.style.plugins}`);
    // const css = callback(file, fname);
    let css = file;
    ...
    writeFile(`${cwd}/build/style/`, `${path.parse(fname).name}.css`, css);
  }
